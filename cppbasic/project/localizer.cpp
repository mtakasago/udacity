...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	// your code here
	int cells = grid.size() * grid[0].size();
  	for(int i=0;i<grid.size();i++)
    {
      	vector<float> row;
		for(int j=0;j<grid[0].size();j++)
        {
          	row.push_back(1.0/cells);
        }
	...
	float p_miss)
{
	...
	for(int i=0;i<beliefs.size();i++)
    {
      	...
      	for(int j=0;j<beliefs[0].size();j++)
        {
          	...
          	float new_belief = beliefs[i][j] * (hit * p_hit + (1-hit) * p_miss);
          	row.push_back(new_belief);
          	new_sum += new_belief;
        }
      	newGrid.push_back(row);
    }
  	for(int i=0;i<newGrid.size();i++) for(int j=0;j<newGrid[0].size();j++) newGrid[i][j] /= new_sum;
